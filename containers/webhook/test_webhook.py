...
def main():
    ...
    parser.add_argument(
        '--url',
        default='http://localhost:8000/api/webhooks/github',
        help='Webhook URL (default: http://localhost:8000/api/webhooks/github)',
    )
    parser.add_argument(
        '--secret', default='', help='Webhook secret for signature verification'
    )
    ...
    if args.secret:
        signature = hmac.new(
            args.secret.encode(), msg=payload_bytes, digestmod='sha256'
        ).hexdigest()
