...
class TestAuthWebhookIntegration:
    ...
    def test_complete_auth_webhook_flow(self, client):
        ...
        update_response = client.patch(
            f'/api/webhooks/configs/{webhook_id}',
            json={
        ...
        get_response = client.get(
            f'/api/webhooks/configs/{webhook_id}', headers=headers
        )
    ...
    def test_webhook_validation(self, client):
        ...
        login_response = client.post(
            '/api/auth/token',
            json={'username': 'webhookuser', 'password': 'password123'},
        )
    ...
    def test_user_isolation(self, client):
        ...
        login1_response = client.post(
            '/api/auth/token', json={'username': 'user1', 'password': 'password123'}
        )
        ...
        login2_response = client.post(
            '/api/auth/token', json={'username': 'user2', 'password': 'password123'}
        )
        token2 = login2_response.json()['access_token']
        headers2 = {'Authorization': f'Bearer {token2}'}

        # User 1 creates a webhook
        webhook1_response = client.post(
            '/api/webhooks/configs',
            json={
                'name': 'User 1 Webhook',
        ...
        access_response = client.get(
            f'/api/webhooks/configs/{user2_webhook_id}', headers=headers1
        )
